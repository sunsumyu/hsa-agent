...
with open("e:/chain/hsa-agent/app/agent_graph.py", "r", encoding="utf-8") as f:
    content = f.read()
...
new_sqlexec = """async def sqlexec_node(state: AuditState, config: RunnableConfig):
    \"\"\"
    [工业�?Skills 节点] 完全基于 Tool Calling 调用独立封装�?Skills�?    \"\"\"
    ...
    if fast_route_id:
        logger.success(f"🚀 [FAST_EXEC] Fast Route 命中，直接执�?Skill")
        try:
    ...
    prompt = prompt_template.format_messages(
        messages=state["messages"], schema_info="请使�?lookup_medical_schema 技能按需查询字段",
...
new_build_graph = """def build_graph():
    \"\"\"
    [工业�?Skills 拓扑] 严格的线性工作流
    PLANNER �?SKILLS_EXEC(原SQLEXEC) �?限次重试) �?CONSOLIDATOR �?REPORTER �?END
    \"\"\"
    workflow = StateGraph(AuditState)
    workflow.add_node("PLANNER", planner_node)
    workflow.add_node("SQLEXEC", sqlexec_node)  # 我们保留了名字以兼容旧代�?    workflow.add_node("REPORTER", reporter_node)
...
# Save
with open("e:/chain/hsa-agent/app/agent_graph.py", "w", encoding="utf-8") as f:
    f.write(content)
