...
核心改进：
    ...
    2. 全量 DDL 分区注入：注入目标表的 **全部字段**（而非 top-k 片段），
       从根本上消除字段幻觉
    3. 分层提示：按数据域分层描述表的用途和约束

设计理念：
    资料分区分层 — 查得快、查得准、不幻觉：
    - 分区：3 张物理表分属不同数据域，不混注
...
class SchemaInjector:
    ...
    def inject(
        ...
        lines.append(
            ...
            f"如需统计值请用 `COUNT(*) AS xxx`, `SUM(col) AS xxx` 形式创建别名。\n"
        )
        ...
        for table_name in target_tables:
            ...
            lines.append(f"\n### 表 `{table_name}` ({len(table_fields)} 字段)")
