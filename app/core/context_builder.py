...
from app.core.context_selector import AdaptiveSelector
...
class ContextBuilder:
    ...
    async def build_optimal_context(
        ...
        packets = self._gather_all(user_query, target_tables, history)
        ...
        # 3. Structure (结构化分区隔离)
        # 我们保留 System 消息和聊天历史的自然流，将 XML 事实载荷作为干净的 SystemMessage 物理隔离地追加到消息流尾部！
    ...
    def _gather_all(self, query: str, target_tables: List[str], history: List[BaseMessage]) -> List[ContextPacket]:
        packets = []
    ...
    def _structure_into_messages(self, selected: List[ContextPacket], original_history: List[BaseMessage]) -> List[BaseMessage]:
        ...
        schemas, memories, histories, notes = [], [], [], []
