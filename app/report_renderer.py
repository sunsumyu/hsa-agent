...
class AuditReportRenderer:
    ...
    def prepare_conclusion_prompt(self, user_question: str, methodology: str, execution_trace: List[str], hard_count: int, hard_sum: float) -> str:
        ...
        if hard_count == 0:
            defense_instruction = (
                "5. **【强制】时序防守深度与边缘场景免责声明**：当前核查结果为 0 条。由于原始指令强制在自然年/月等静态周期检索，你**绝对禁止**仅回答“未发现异常”。你**必须**在结论的末尾追加一段高度专业的关于时间切分边缘场景的防御性审查声明，话术应与以下模板高度对齐：\n"
                "   “本次核查基于指定的静态自然时间域（如2024年/月）执行全量扫描，结果为 0。但鉴于医保黑产常采用跨自然年/自然月边界的【骑缝购药】或利用微秒级【高频拆单】以规避静态聚合风控，本次基于自然周期的结论存在逻辑截断风险。为保障基金绝对安全，建议后续叠加 连续 30 天滑动窗口 (Sliding Window) 及 单笔交易时间戳差值 (Lag Diff) 规则进行动态时序复核。”\n"
