...
class CharsetProxy:
    ...
    def command(self, sql):
        """[V185.0] 驱动无关命令接口：自动兼容 Native (driver) 和 HTTP (connect) 执行 DDL"""
        try:
    ...
    def insert(self, table, data, column_names=None):
        """[V185.0] 驱动无关数据批量导入接口：自动兼容 Native 和 HTTP 插入临时表"""
        try:
            if hasattr(self.client, 'insert'):
                ...
                return self.client.execute(sql, data)
        except Exception as e:
            clean_msg = self._clean_error_message(str(e))
            raise SqlExecError(clean_msg)
...
def get_clickhouse_client():
    ...
    database = _os.getenv("CLICKHOUSE_DB", "fqz_hsa")
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            if s.connect_ex((host, 9000)) == 0:
                ...
                client = clickhouse_driver.Client(
                    host=host, port=9000, user=user, password=password, database=database,
            ...
            for wsl_ip in wsl_ips:
                try:
                    ...
                    client = clickhouse_connect.get_client(
                        host=wsl_ip, port=8123, username=user, password=password, database=database,
                        connect_timeout=3
                    )
    ...
    if not client:
        try:
            ...
            client = clickhouse_connect.get_client(
                host='localhost', port=8123, username=user, password=password, database=database,
                connect_timeout=3
            )
